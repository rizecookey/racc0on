...
public class x8664StoreAllocator {
    ...
    public Allocation allocate(List<x8664Op> sequentialProgram, StoreRequests<x8664Op, x8664Store> storeRequests) {
        LivenessMap<x8664Op, x8664Store> liveness = LivenessMap.calculateFor(sequentialProgram, storeRequests);
        InterferenceGraph<x8664Op, x8664Store> interference = InterferenceGraph.createFrom(sequentialProgram, liveness, storeRequests);
